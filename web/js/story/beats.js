 ...
 * caption, camera and frame without a browser. That matters more here than
 * usual: the thing most likely to rot in a talk deck is the writing, and a test
 * is the only thing that will hold the line on caption length once slides start
 * getting edited ten minutes before a session.
 *
 * ---
 *
 ...
 * that is already in use in the captions below: footprint -> "what it can
 * smell", emissions -> "where it comes from", enhancement -> "a lot".
 */
export const BANNED_WORDS = [
  'footprint', 'flux', 'enhancement', 'inversion', 'inverse', 'ppb', 'baseline',
  'sensitivity', 'lagrangian', 'dispersion', 'advection', 'mole fraction',
  'anomaly', 'boundary layer', 'concentration', 'timeseries', 'parameterisation',
];
 ...
 */
export function resolveFrames({ search = '', store = null, nTime = Infinity } = {}) {
  const frames = { ...FRAMES };
 ...
 *
 * @param {object} frames  resolved FRAMES
 * @param {object} opts    {showRecordLow, hasWind}
 */
export function buildBeats(frames = FRAMES, {
